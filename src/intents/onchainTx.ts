...
export async function lookupTransaction(
  ...
  const [tx, receipt, headHex] = await Promise.all([
  ...
  const pending = blockNumber === null || !receipt;
  const succeeded = receipt?.status === '0x1';
  const status: TxLookupResponse['status'] = pending ? 'pending' : succeeded ? 'success' : 'failed';
